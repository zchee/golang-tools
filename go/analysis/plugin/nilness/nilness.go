...
// run inspects the control-flow graph of each SSA function in a package
// and returns a list of errors such as nil pointer dereferences and
...
func run(unit *analysis.Unit) error {
	...
	for _, fn := range ssainput.SrcFuncs {
		unit.Findings = append(unit.Findings, runFunc(fn)...)
	}
	return nil
}

func runFunc(fn *ssa.Function) (findings []*analysis.Finding) {
	...
	notNil := func(stack []fact, instr ssa.Instruction, v ssa.Value, descr string) {
		if nilnessOf(stack, v) == isnil {
			findings = append(findings, &analysis.Finding{
				Pos:      instr.Pos(),
				Category: "nilderef",
				Message:  "nil dereference in " + descr,
			})
		}
	}
		...
		if binop, tsucc, fsucc := eq(b); binop != nil {
			...
			if ynil != unknown && xnil != unknown && (xnil == isnil || ynil == isnil) {
				...
				findings = append(findings, &analysis.Finding{
					Pos:      binop.Pos(),
					Category: "cond",
					Message:  fmt.Sprintf("%s condition: %s %s %s", adj, xnil, binop.Op, ynil),
				})
