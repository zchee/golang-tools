...
var AssemblyAnalysis = &analysis.Analysis{
	Name: "asmdecl",
	Doc:  "check assembly against Go declarations",
	Run:  runAssembly,
}
...
func runAssembly(unit *analysis.Unit) error {
	// TODO: we need a way to obtain the names of non-Go files (or
	// perhaps all non-selected files, .go included) from the unit.
	// Whatever we specify must be implementable by "go vet", which
	// sees only the compilation unit (but not assembly files, among
	// others. And we may need to read their contents, ideally at
	// most once, and we may need to create a token.File to report errors.
	var nonGoFiles []string // TODO
	...
	for _, f := range unit.Syntax {
		for _, decl := range f.Decls {
			if decl, ok := decl.(*ast.FuncDecl); ok && decl.Body == nil {
				knownFunc[decl.Name.Name] = asmParseDecl(unit, decl)
			}
		}
	}
...
Files:
	for _, fname := range sfiles {
		// TODO: make it easier to read raw files and report
		// error against them. See lineStart helper below.
		content, err := ioutil.ReadFile(fname)
		if err != nil {
			return err
		}
		tf := unit.Fset.AddFile(fname, -1, len(content))
		...
		flushRet := func() {
			if fn != nil && fn.vars["ret"] != nil && !haveRetArg && len(retLine) > 0 {
				...
				for _, line := range retLine {
					unit.Findingf(lineStart(tf, line), "[%s] %s: RET without writing to %d-byte ret+%d(FP)", arch, fnName, v.size, v.off)
				}
			}
			retLine = nil
		}
		for lineno, line := range lines {
			...
			badf := func(format string, args ...interface{}) {
				unit.Findingf(lineStart(tf, lineno), "[%s] %s: %s", arch, fnName, fmt.Sprintf(format, args...))
			}
...
func asmParseDecl(unit *analysis.Unit, decl *ast.FuncDecl) map[string]*asmFunc {
	var (
		arch   *asmArch
		fn     *asmFunc
		offset int
	...
	addParams := func(list []*ast.Field, isret bool) {
		...
		for _, fld := range list {
			t := unit.Info.Types[fld.Type].Type
...
func lineStart(f *token.File, line int) token.Pos {
	// We use binary search to find the start offset of this line.
	// This could be implemented more simply and efficiently
	// given direct access to f.lines.
	// CL https://go-review.googlesource.com/c/go/+/134075
	// adds (*File).LineStart to go/token.
