...
// runUnreachable checks function bodies for dead code.
...
func (d *deadState) findLabels(stmt ast.Stmt) {
	switch x := stmt.(type) {
	default:
		log.Fatalf("%s: internal error in findLabels: unexpected statement %T", d.unit.Fset.Position(x.Pos()), x)
...
func (d *deadState) findDead(stmt ast.Stmt) {
	...
	if !d.reachable {
		...
		default:
			d.unit.Findingf(stmt.Pos(), "unreachable code")
	...
	switch x := stmt.(type) {
	default:
		log.Fatalf("%s: internal error in findDead: unexpected statement %T", d.unit.Fset.Position(x.Pos()), x)
