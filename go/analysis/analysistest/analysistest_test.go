...
func main() {
	...
	println() // trigger an unexpected finding
...
`}
	...
	analysistest.Run(t2, dir, findcall.Analysis, "a")

	want := []string{
		`a/b.go:8:10: in 'want' comment: invalid syntax`,
		`a/b.go:5:9: finding "call of println(...)" does not match pattern "wrong expectation text"`,
		`a/b.go:6:9: unexpected finding: call of println(...)`,
		`a/b.go:7: expected finding matching "unsatisfied expectation"`,
	}
