...
func opendir(path string) (dir uintptr, err error) {
	// We implent opendir so that we don't have to call open(2) then
	// fdopendir(3). The returned DIR* is opened with O_CLOEXEC.
	p, err := syscall.BytePtrFromString(path)
	if err != nil {
		return 0, err
	}
	dir, _, e1 := syscall_syscallPtr(libc_opendir_trampoline_addr, uintptr(unsafe.Pointer(p)), 0, 0)
	if e1 != 0 {
		err = e1
	}
	return dir, err
