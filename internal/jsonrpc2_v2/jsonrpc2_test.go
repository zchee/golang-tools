...
type cancelParams struct{ ID jsonrpc2.ID }
	...
	for _, test := range callTests {
		t.Run(test.Name(), func(t *testing.T) {
			client, err := jsonrpc2.Dial(ctx,
				listener.Dialer(ctx), binder{framer, func(h *handler) {
					defer h.conn.Close()
...
func (test call) Invoke(t *testing.T, ctx context.Context, h *handler) {
	...
	if err := h.conn.Call(ctx, test.method, test.params, results); err != nil {
...
func (test echo) Invoke(t *testing.T, ctx context.Context, h *handler) {
	results := newResults(test.expect)
	if err := h.conn.Call(ctx, "echo", []interface{}{test.method, test.params}, results); err != nil {
...
func (test async) Invoke(t *testing.T, ctx context.Context, h *handler) {
	o, err := h.conn.AsyncCall(ctx, test.method, test.params)
	if err != nil {
		t.Fatalf("%v:Async failed: %v", test.method, err)
	}
	h.calls[test.name] = o
}
...
func (test cancel) Invoke(t *testing.T, ctx context.Context, h *handler) {
	o := h.calls[test.name]
	if err := h.conn.Notify(ctx, "cancel", &cancelParams{o.ID()}); err != nil {
		t.Fatalf("%v:Collect failed: %v", test.name, err)
	}
}
...
func (b binder) Bind(conn *jsonrpc2.Connection) (jsonrpc2.ConnectionOptions, error) {
	h := &handler{
		conn:       conn,
...
func (h *handler) Handle(ctx context.Context, req *jsonrpc2.Request) (interface{}, error) {
	...
	case "echo":
		...
		err := h.conn.Call(ctx, v[0].(string), v[1], &result)
		return result, err
	case "wait":
		var name string
		if err := json.Unmarshal(req.Params, &name); err != nil {
