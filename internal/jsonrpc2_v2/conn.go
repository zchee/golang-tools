...
type Binder interface {
	...
	Bind(*Connection) (ConnectionOptions, error)
}
...
type ConnectionOptions struct {
	...
	// If nil, all messages will be not handled.
	Handler Handler
}
...
func (o ConnectionOptions) Bind(*Connection) (ConnectionOptions, error) {
	return o, nil
}
...
// It is equivalent to an AsyncCall immediately followed by a call to CollectResponse with the
// returned ID, and is the preferred way to make calls safely if you want to immediately block
// on the response.
// The params will be marshaled to JSON before sending over the wire, and will
// be handed to the method invoked.
// The response will be unmarshaled from JSON into the result.
func (c *Connection) Call(ctx context.Context, method string, params, result interface{}) error {
	o, err := c.AsyncCall(ctx, method, params)
	if err != nil {
		return err
	}
	return o.Collect(ctx, result)
}

// AsyncCall does not wait for the response like Call.
// Instead it returns an AsyncCall that can be used for collecting the response and cancelling.
func (c *Connection) AsyncCall(ctx context.Context, method string, params interface{}) (*AsyncCall, error) {
	// generate a new request identifier
	id := Int64ID(atomic.AddInt64(&c.seq, 1))
	call, err := NewCall(id, method, params)
	if err != nil {
		return nil, errors.Errorf("marshaling call parameters: %w", err)
	}
	...
	if err := c.write(ctx, call); err != nil {
		// sending failed, we will never get a response, so don't deliver a fake one
		r, _ := NewResponse(id, nil, err)
...
// Collect the results of an AsyncCall.
// It must be called exactly once for each AsyncCall.
// It will fill in result in the same way the blocking Call would.
func (a *AsyncCall) Collect(ctx context.Context, result interface{}) error {
	defer a.endSpan()
	//TODO: should we hold onto the span ctx in order to do the labelling below?
	select {
...
// handler has not previously returned ErrAsyncResponse. It is also an error to
...
// cancelled.
func (c *Connection) Cancel(ctx context.Context, id ID) {
	pending := <-c.incomingBox
	defer func() { c.incomingBox <- pending }()
