...
func (l *netListener) Dialer(ctx context.Context) Dialer {
	return NetDialer(ctx, l.net.Addr().Network(), l.net.Addr().String(), net.Dialer{
		Timeout: 5 * time.Second,
	})
}

// NetDialer returns a Dialer using the supplied standard network dialer.
func NetDialer(ctx context.Context, network, address string, nd net.Dialer) Dialer {
	return &netDialer{
		network: network,
		address: address,
		dialer:  nd,
...
func (l *netPiper) Dialer(ctx context.Context) Dialer {
	return l
}
