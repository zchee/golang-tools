...
func ClientDispatcherV2(conn *jsonrpc2_v2.Connection) Client {
	sender := clientConnV2{conn}
	return &clientDispatcher{sender}
}
...
func ClientHandlerV2(client Client) jsonrpc2_v2.Handler {
	return jsonrpc2_v2.HandlerFunc(func(ctx context.Context, req *jsonrpc2_v2.Request) (interface{}, error) {
		...
		replier := func(ctx context.Context, res interface{}, err error) error {
...
func req2to1(req2 *jsonrpc2_v2.Request) jsonrpc2.Request {
	if req2.ID.IsValid() {
		idv2json, err := json.Marshal(req2.ID)
		if err != nil {
			panic(err)
		}
		var idv1 jsonrpc2.ID
		if err := json.Unmarshal(idv2json, &idv1); err != nil {
			panic(err)
		}
